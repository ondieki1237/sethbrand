...
export async function handleWeeklyReport(req: NextRequest) {
  if (!authorize(req)) {
  ...
  try {
    ...
    const formatList = (items: { _id: string | null; count: number }[]) =>
      items
    ...
    return NextResponse.json({ success: true, totalViews, totalLeads })
  } catch (error) {
